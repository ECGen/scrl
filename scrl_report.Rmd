...
ard.dif <- apply(ard.dif, 2, function(x) x / max(x))
...
pdf(file = "./results/plot_ard_isp.pdf", width = 9, height = 5)

par(mfrow = c(1,2))
bp.out <- barplot(ard.mu, col = "darkgrey", ylim = c(-0.4, 0), 
...
v.ard.dif <- apply(v.ard.dif, 2, function(x) x / max(x))
...
par(mfrow = c(1,2))
bp.out <- barplot(v.ard.mu, col = "darkgrey", ylim = c(-3.0, 0), 
                  ylab = "Relativized Difference (S - R)", border = "NA")
