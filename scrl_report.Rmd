...
l.d <- cbind(le.raw[, -2:-3], l.raw[, -1:-3])
...
moth.alive <- lapply(trt, function(x) x[x %in% c(letters, LETTERS)])
moth.alive <- do.call(rbind, moth.alive)
tree <- lapply(trt, function(x) x[!(x %in% c(letters, LETTERS))])
...
v.com <- v.com[, apply(v.com, 2, sum) > 0]
...
```{r veg-rel-moth-permtab}

kable(ptab.v.moth.rel)
...
```{r veg-rel-env-permtab}

kable(ptab.v.env.rel)
...
```{r veg-env-seq-permtab}

kable(ptab.v.env.seq)

```

```{r veg-rel-env-seq-permtab}

kable(ptab.v.env.rel.seq)
...
susceptibility, Apahe plume and *Asteraceae ovales*. 


```{r isp-veg-table}

kable(isp.all.v[, 1:6], digits = 4)
